...
describe("회원가입 폼 테스트", () => {
  ...
  context("사용자 경험 테스트", () => {
    ...
    context(
      "필수입력필드를 입력하지 않고 넘어가면 사용자에게 피드백을 준다.",
      () => {
        beforeEach("input에 값을 입력했다가 지운다.", () => {
          cy.get("#name").type(`용상윤`);
          cy.get("#id").type(`ysy`);
          cy.get("#email").type(`ysy@naver.com`);
          cy.get("#password").type(`123123`);
    ...
    context("비밀번호에는 '암호 수준'이 있다.", () => {
      it("1. '금지된 수준': 0 자리 초과일 경우", () => {
        cy.get("#password").type(`abcdef`);
        anyClicked();
        cy.get("#field-password label")
          .last()
          .should("have.text", "금지된 수준");
      });

      it("2. '심각한 수준': 12 자리 초과일 경우", () => {
        cy.get("#password").type(`qwerqwerqwerq`);
        anyClicked();
        cy.get("#field-password label")
          .last()
          .should("have.text", "심각한 수준");
      });

      it("3. '보통 수준': 특수문자가 포함될 경우", () => {
        cy.get("#password").type(`qwerqwerqwer!`);
        anyClicked();
        cy.get("#field-password label").last().should("have.text", "보통 수준");
      });

      it("4. '강력한 암호': 숫자가 포함될 경우", () => {
        cy.get("#password").type(`qwerqwerqwer!123`);
        anyClicked();
        cy.get("#field-password label")
          .last()
          .should("have.text", "강력한 암호");
      });
    });
  });
  ...
  context("유효성 검증 테스트", () => {
    ...
    context("비밀번호 확인", () => {
      ...
      it("2. 동일하지 않은 값일 경우 초록불이 꺼지고, 메세지가 출력하는지 확인", () => {
        cy.get("#password").type(`qwer123!@#`);
        cy.get("#password-check").type(`qwer123!@##`);
        anyClicked();
        cy.get("#field-password .check").should("have.class", "text-green-500");
      ...
      it("3. 빈 값을 입력했을 때는 필수입력항목이라는 메세지를 출력", () => {
        cy.get("#password").type(`qwer123!@#`);
        cy.get("#password-check").type(`qwer123!@##`);
        anyClicked();
        cy.get("#password-check").clear();
